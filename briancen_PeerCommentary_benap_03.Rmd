...
> BP: Remember the formatting guidelines!
...
> BP: Love that you're just loading the csv directly from github, and how you're using a for loop to run through these parameters for each of the variables. I just did each one individually, but I think your way here is probably better.
...
> BP: Love that you're using facet wrap to plot everything together, though the skyblue & pink color scheme is kinda straining on my eyes. Maybe consider outlining in a darker color?
...
>BP: Small thing, but I'm interested in your choice of colors here since pink and purple are so close and might be slightly difficult to parse for some people.
...
```
Response: Height, weight and age seems to have normal distribution. Zombie kills and years of education are right skewed and they consist of discrete counting number. Thus, it suggest that the samples are from poisson distribution.
#Task5：sample 30
```{r}
set.seed(888)          
...
> BP: Looks good! I think my main feedback is just to make sure you're following the formatting guidelines and to maybe include a bit more commenting on what each part of your code (i.e. arguments you're passing to functions etc.) is doing, especially for the last two questions which can be kind of difficult to follow along on. That said you do have them all broken up in a very legible way!
